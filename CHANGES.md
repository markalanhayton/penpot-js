...
#### backend-js: Database migration parity (0010–0015)
...
- Begin Phase 1 of Clojure/ClojureScript → ES JavaScript migration: port 7 foundational modules from `common/` to `common-js/` — `i18n`, `version`, `path_names`, `buffer`, `perf`, `pprint`, `schema` (hand-built validators replacing Malli dependency)
- Continue Phase 1 migration: port 6 more modules from `common/` to `common-js/` — `uri`, `spec` (spec/assertions replacing clojure.spec), `thumbnails`, `record` (defrecord → factory pattern), `data/macros` (select-keys, get-in, fmt), `data/undo_stack`
...
- Refactor switch/case dispatch patterns to Map-based lookups across common-js, backend-js, frontend, common, backend, and exporter modules for improved maintainability and O(1) lookup performance. Affected modules: modifiers, features, media, constraints, shape_type (common-js); dispatcher, media, transit, scheduler (backend-js); viewport/utils, http, serializers, shape_icon, parser, format, text_editor, fonts (frontend); shape, interactions, media, logging, features, variants (common); gc_touched, rlimit (backend); mime (exporter).
