...
- Fix `isPointInGeomDataQ` crash on line-only content where `rayLineIntersect` returns null and `.flat()` passes it through — `.filter(Boolean)` now strips nulls before accessing `[1]`
- Fix boolean operations for concave shapes — rewrite `bool-ops.js` with convex decomposition for intersection, point-in-polygon containment, and even-odd fill for difference/exclusion; concave shapes (L-shapes, stars, arrows) now work correctly (WU-Q7)
