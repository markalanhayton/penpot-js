...
- **Server: `get-audit-events` RPC + Client Audit Log tab (WU-T5)**: New server RPC in `server/src/rpc/audit.js` returns paginated, filterable audit log events with filters `profileId`, `eventType`, `eventName`, `source`, `teamId` (joined via `team_profile_rel`), `from`/`to` (ISO timestamp bounds on `tracked_at`), `includeArchived` (default false). Pagination via `limit` (default 50, max 200) and `offset`. Returns `{ events: [...], total: N }` where `total` is the unpaginated row count. Events are camelCased via `rowToCamel`; `props` and `context` are auto-parsed as JSON. Returns empty when the `audit-log` flag is disabled. The `audit_log` table already has the right indexes for the new filter combinations. New "Audit Log" tab in `penpot-settings.js` provides a filterable, paginated event list. 30 new tests (16 server + 15 client unit + 4 E2E), all passing.
- **Client: Team ownership transfer workflow (WU-T1)**: `penpot-team-management.js` adds a "Transfer Ownership" section in the settings tab, owner-only, hidden when the team has only one member. Clicking opens a modal with a member `<select>` dropdown (self excluded) and ARIA `dialog` semantics. Confirm calls `update-team-member-role` twice in sequence (new owner → `role:'owner'`, previous owner → `role:'admin'`) and pushes a `transfer-ownership` audit event with `teamId`/`previousOwnerId`/`newOwnerId`/emails for traceability. Browser `confirm()` dialog precedes the modal confirm. Escape key and backdrop click dismiss the modal. 24 new tests (13 client unit + 9 E2E + 2 server integration), all passing.
- **Client: Multi-step onboarding (WU-T2)**: `app.js` now runs `runOnboardingFlow(profile)` after successful login. The flow shows two new overlays (only for first-time users with no `onboardingViewed` prop): (1) `penpot-onboarding-questions` asks three questions (role, team size, use case) and persists answers via `update-profile-props` with the new `onboarding-{role,team-size,use-case}` allow-keys (server allow-list extended in `server/src/rpc/profile.js`); (2) `penpot-onboarding-team-choice` shows a Create-Team / Join-via-invite / Skip prompt when the user has 0 teams. Both flows best-effort and localStorage-flagged so they don't re-show. 30 new tests (2 server + 19 client unit + 9 E2E), all passing.
- **Client: Team form with logo, description, color (WU-T3)**: New `penpot-team-form` custom element with create + edit modes — 4 fields (name, description with ≤500-char live counter, color with 15-swatch palette + custom input, logo upload via existing `update-team-photo` RPC). Modal overlay with backdrop/Escape dismissal. Server `create-team` and `update-team` RPCs now validate the `features` sub-keys: `description` is a string clamped to 500 chars, `color` must match `/^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/`; both stored in `team.features` (JSON column). RPCs also auto-parse the returned `features` from a JSON string to an object. `penpot-team-management.js` settings tab now has an "Open Edit Form" button (admin+ only) that opens the form pre-filled. 38 new tests (7 server + 21 client unit + 10 E2E), all passing.

### :bug: Bugs fixed

- **Client: Fix RpcError defaults and shared package import paths**: `RpcError` constructor now defaults `status` to 400 and accepts an optional `extra` metadata argument, matching the AGENTS.md requirement that error callers include type, code, and hint. Two shared-package imports (`transit.js` and `shapes.js`) were using the subpath `./uuid.js` while the package's `exports` map only exposes `./uuid` (no extension). `client/test/rpc-unit.test.js` was using top-level `await import()` inside `describe` callbacks (a syntax error); hoisted to static ESM imports at module top.
- **Client: Fix toolbar title display, icon CSS var, and zoom button layout**: Toolbar title was always showing the template's "Untitled file" because the editable-text check used `!el.contentEditable`, which evaluates to false for the default `'inherit'` value. `penpot-icon` now normalizes bare numeric sizes by appending `px` so the CSS variable resolves to a valid length. Zoom controls in the tools bar now use flexbox centering and a minimum width for consistent button size.
...
- **Client: UI layout and z-index system overhaul**: Replace all hardcoded z-index values (50–2000) across 20+ components with CSS custom property tokens (`--penpot-z-*`) establishing a proper stacking order: canvas(0) → overlay(1) → panels(100) → guides(200) → set(300) → dropdown(400) → context-menu(500) → modal(600) → tooltip(700) → notification(800) → loaders(900) → overlay(1000). Fix modal/selection-set z-index collision (both were 300). Fix mobile sidebar z-index (was z:1, now z:600) so sidebars render above panels and dropdowns. Fix MCP/plugin panel positioning from hardcoded `right:270px` to `calc(var(--penpot-sidebar-width) + 10px)` for responsive sidebar width. Replace `float:right` in comment panel with `margin-left:auto` flex layout. Add responsive `max-width` guards to all dialog components (share, import, export, shortcuts, modal) preventing viewport overflow on mobile. Convert workspace canvas wrapper inline styles to CSS class `.penpot-app__canvas-wrapper` with `overflow:hidden`. Fix notification z-index from 120 to 800 (above modals). Fix text toolbar z-index from 50 to 400 (above panels). Fix `.penpot-visible-mobile` from `display:block !important` to `display:flex !important` to preserve button alignment on mobile.
- **Client: Visual regression testing (PA-18)**: Add screenshot-based visual regression test suite using Playwright `toHaveScreenshot()`. 25 screenshot comparison tests covering auth screen (login, register, recovery, error states), dashboard, workspace shell (toolbar, tools bar), sidebars (left, right empty/selected), canvas (empty, with shapes), full-page workspace, and 8 design system components (button, input, checkbox, switch, radio, badge, avatar, loader). Baselines stored in `client/e2e/screenshots/` with `snapshotPathTemplate` config. CI fails on >2% pixel difference. Baselines updateable via `npx playwright test --update-snapshots`.
- **Server: `update-file-library-sync-status` RPC (SA-2)**: New RPC handler that creates or updates a `file_library_sync` row recording when a file-library sync was acknowledged. Uses `INSERT ... ON CONFLICT DO UPDATE` to upsert the `synced_at` timestamp. Checks edition permissions on both the consumer file and the library file. 3 integration tests (create, update, unauthorized).
- **Server: `ignore-file-library-sync-status` RPC (SA-1)**: New RPC handler that sets `ignore_sync_until` on a file to suppress library sync notifications. Checks edition permissions before updating. Returns full file row via `RETURNING` clause. 4 integration tests (set, update, unauthorized, deleted file).
- **Server: `get-file-media-objects` RPC**: New RPC handler that lists all non-deleted media objects for a file. Returns camelCase rows from `file_media_object` table.
- **Server: Test coverage expansion**: 19 new RPC handler test files covering auth, comments, demo, export, feedback, files, files-update, files-snapshots, files-thumbnails, ldap, management, media, nitrate, oidc, profile, projects, teams, verify-token. Total server tests: 879 (from 529), all passing.
- **Server: `'use strict'` directives**: Added to all RPC modules, middleware, tasks, storage, transit, WebSocket, and setup files for strict mode compliance.
- **Shared: Text content tree utilities**: New exports in `shared/src/types/text.js`: `createDefaultContent()`, `contentToPlainText()`, `isContentTree()`, `decorateRangeInfo()`, `updateTextRange()`, `updateRootAttrs()`, `updateParagraphAttrs()`, `updateTextAttrs()`, `currentRootAttrs()`, `currentParagraphAttrs()`, `currentTextNodeAttrs()`, `mergeTextNodeAttrs()`. 10 new unit tests.
- **Shared: SVG parser robustness**: `svg_parser.js` now validates command characters with a `VALID_COMMANDS` set and rejects unrecognized characters. `isPointInGeomDataQ()` in `helpers.js` now filters `null`/`undefined` values from the intersection test pipeline.
- **Shared: `constants` export**: New barrel export `@penpot/shared/constants` for shared constants.

- **Text v3 per-range styles (PA-5)**: Implement the upstream content tree model for text shapes, enabling per-range and per-paragraph inline styles (font family, size, weight, style, text decoration, text transform, fills, letter spacing, line height). New `lib/content-tree.js` provides `contentTreeToHTML()` and `htmlToContentTree()` for bidirectional conversion between the penpot content tree format and contentEditable HTML. `shared/src/types/text.js` extended with `createDefaultContent()`, `updateTextRange()`, `updateTextAttrs()`, `updateParagraphAttrs()`, `updateRootAttrs()`, `currentTextNodeAttrs()`, `currentParagraphAttrs()`, `decorateRangeInfo()`, `isContentTree()`, `contentToPlainText()`. SVG rendering now traverses content trees with `<tspan>` elements per text node style run (per-segment fills, font-size, font-weight, font-style, text-decoration, text-transform). Canvas2D renderer renders per-segment font and fill styles. Rich text editor (`lib/rich-text.js`) commits content trees on editing complete, preserving per-range styles from `document.execCommand` formatting. Text shapes created with `{type: 'root', children: [...]}` content tree. Font/alignment property changes update both shape-level props and content tree. `lib/export.js` and `lib/thumbnail.js` updated to handle content trees. 10 new unit tests in `shared/test/types/text.test.js`.

- **Main menu bar (PA-3)**: Add `penpot-main-menu` component with File/Edit/View dropdown menus to the workspace toolbar. Supports keyboard mnemonics (Alt+F/E/V), hover-to-switch between menus, checked state for toggle items (Rulers, Grid, Snap), submenu support, and full integration with workspace actions (Save, Export, Import, Undo/Redo, Copy/Paste/Cut, Zoom, Comments, Version History, Shortcuts reference). Menu actions dispatch `penpot-menu-action` events handled by the workspace.

- **System clipboard (PA-4)**: Implement system clipboard integration for copy/paste/cut operations. New `lib/clipboard.js` module provides `copyShapesToClipboard()`, `readShapesFromClipboard()`, `deepCloneShape()`, and `assignNewIds()` for serializing shapes as JSON to the system clipboard via `navigator.clipboard.writeText()`. Workspace now intercepts `copy`/`cut`/`paste` DOM events: copy writes selected shapes as `application/json+penpot` to system clipboard; cut copies then deletes; paste handles penpot shape data, SVG text, and image files from the system clipboard, and falls back to creating text shapes from plain text. Deep cloning ensures nested arrays (fills, strokes, shadows, points) are properly isolated. Ctrl+C/V/X removed from keyboard shortcut handler to allow native clipboard events to propagate.

- **Custom canvas scrollbars (PA-9)**: Add `penpot-scrollbars` overlay component for vertical and horizontal scrollbars on the canvas. Scrollbars appear when content extends beyond the viewport, with thumb size proportional to the viewport/content ratio. Drag-to-pan interaction dispatches `penpot-scrollbar-pan` events that update canvas `panX`/`panY`. Scrollbars auto-hide when all content fits. Integrated with canvas zoom/pan changes and `renderCurrentPage()` via `#updateScrollbars()` in workspace.

- **Fix deleted fonts (PA-10)**: Add `lib/fix-deleted-fonts.js` module that detects text shapes and typographies referencing fonts no longer available in the team's font library. On file load, `fixDeletedFontsForLibrary()` checks for invalid typography font-id references and auto-fixes them by substituting a valid font-id with matching font-family. On each page load, `fixDeletedFontsForPage()` checks text shape content trees for invalid font-id references and emits `mod-obj` changes with corrected content and cleared `position-data`. A warning banner in the right sidebar shows which fonts are missing when a text shape with an invalid font-id is selected. Right sidebar font dropdown now uses the canonical `SYSTEM_FONTS` constant instead of a hardcoded list. `shared/src/types/text.js` re-export added to import map for `@penpot/shared/types/text.js`.

- **Multi-select bounding box (PA-11)**: Add group bounding box with resize and rotation handles for multiple selected shapes. `lib/shapes.js` gains `computeShapesBounds()` to compute the union bounding rect of selected shapes. SVG renderer (`renderPage`) and Canvas2D renderer both render individual purple outlines per selected shape plus a dashed green group bounding rect with 8 resize handles and a rotation handle. `SelectTool` in `tools/base.js` now supports multi-select resize (proportional scaling of all shapes relative to group bounding box) and multi-select rotation (orbits each shape around the group center). Hit-testing for handles and rotation work for both single and multi-select.

- **Templates tab server-side (WU-Q6)**: Implement `get-builtin-templates` and `clone-template` RPC handlers in `server/src/rpc/management.js`. Templates are loaded from `server/resources/onboarding.json` (15 templates matching upstream Penpot: Design tokens starter kit, Penpot Design System, Wireframe library, Prototype template, UI mockup example, Tutorial for beginners, Lucide Icons, Font Awesome, Black & White Mobile Templates, Avataaars, UX Notes, Whiteboarding Kit, Open Color Scheme, Flex Layout Playground, Welcome). `clone-template` downloads `.penpot` files from GitHub URLs and imports them via the existing binfile import pipeline (`parseImportBuffer`/`importParsedFiles`). `PENPOT_TEMPLATES_PATH` env var allows custom template config override. Client `penpot-dashboard.js` now correctly handles array response from `clone-template`. Extracted `importParsedFiles` from `binfile.js` for reuse. 9 new tests covering template listing, validation, permissions, file duplication, and file moving.

- **Ruler guides (WU-C2)**: Add guide creation by dragging from ruler zones, guide line rendering with position labels on hover, guide snap-to-shape integration, and guide persistence via `set-guide` changes. New `penpot-guide-overlay.js` component renders guide lines on canvas with drag-to-move and creation drag zones. `penpot-rulers.js` enhanced with guide creation drag zones and guide position markers. Snap system (`lib/snap.js`) now includes ruler guide snap points. Fixed `process-changes.js` guide handler bug (was using `gridType` instead of `axis`). 10 new unit tests for guide changes and snap points.
- **Complete SVG-to-shapes builder (WU-S3)**: Fully port `shared/src/files/shapes_builder.js` from upstream `shapes_builder.cljc` — the canonical SVG import parser that converts SVG elements into Penpot shape trees. Implements recursive `createSvgChildren` for nested group processing, all shape creation functions (`createRectShape`, `createCircleShape`, `createPathShape`, `createImageShape`, `createGroup`), SVG attribute extraction (`setupFill`, `setupStroke`, `setupOpacity`, `setupOther`), `<use>` tag resolution with deep merge, gradient stop processing, and `strokeOnlySvgPathQ` with upstream-matching fill priority semantics. Fixes 7 bugs from the initial stub: recursive child processing (was only 2 levels deep), `setupOther` display:none target swap, `strokeOnlySvgPathQ` fill priority, `<use>` tag shallow merge, `setupFill` fill-opacity combined clauses, `inheritAttributes` shallow merge, and `createImageShape` redundant calls. `inheritAttributes` in `svg.js` also switched from shallow spread to deep merge. 70 unit tests passing.

- **Nested drag-drop in layer panel (H16)**: Layer panel now supports dragging shapes into/out of frames and groups to reparent them. Three drop zones: "before" (top 6px) and "after" (bottom 6px) for sibling reorder, "into" (center) for reparenting into containers. Ancestor cycle prevention blocks dropping a parent into its own descendants. `mov-objects` change type persisted to server via `makeMoveChange`. Visual feedback: green outline for "into" drops, indented line indicator for "before/after". Works with both flat-map (`page.objects` as `{ id: shape }` with `parentId`/`shapes`) and nested-array data formats. `isBool` now recognized as a container type alongside frames and groups.
- **File thumbnails in dashboard (H1)**: Dashboard fetches existing file thumbnails from server via `get-file-object-thumbnails` RPC on load. For files without thumbnails, generates client-side canvas thumbnails using `generateAndUploadThumbnail` and uploads via `create-file-object-thumbnail` RPC. `drawShapeToCanvas` now renders nested frame/group/bool children and per-corner border radius. `penpot-file-thumbnail` component updated with loading state and fallback icons. Workspace triggers thumbnail regeneration after file save. `penpot-file-grid` falls back to `file.thumbnail` in addition to `file.thumbnailUrl`.
- Add real-time collaboration with operational transform: `lib/ot.js` implements attribute-level OT for `mod-obj` change types (same-shape, same-attribute conflicts are resolved by value swapping to maintain TP1 commutativity); `lib/process-changes.js` provides full 35+ change type processing on the frontend matching the backend engine; `lib/collaboration.js` now uses the undo/reapply pattern from the original Penpot ClojureScript — pending local changes are undone, remote changes applied, then local changes re-applied with OT adjustment; `lib/persistence.js` normalizes change formats and exposes pending changes for coordination; adds `get-file-changes` RPC command to server for lagged-change fetch; 35 unit tests pass across process-changes and OT modules
- Add Node.js exporter service (`server/exporter/`): Playwright-based rendering to PNG, JPEG, WebP, SVG, and PDF; browser pool with configurable min/max; `export-shapes` and `export-frames` handlers; HTTP API on port 6061; Redis pub/sub for progress notifications; zip archive for multi-export; resource upload to server via management API; SVG foreignObject rasterization; pdfunite for multi-frame PDF merging; 22 unit tests for config, URL building, export grouping, and context options; adds `export`, `export-shapes`, and `export-frames` RPC proxy commands to server

#### server: Database migration parity (0010–0015)

Six new SQLite migrations achieving full PostgreSQL schema parity:

- **0010**: Audit gap remediation — 1 missing column (`file_object_thumbnail.media_id`), 5 unique constraints (`comment_thread(file_id,seqn)`, `team_invitation(team_id,email_to)`, `team_invitation(org_id,email_to) WHERE team_id IS NULL`, `team_profile_rel(team_id,profile_id)`, `team_project_profile_rel(team_id,project_id,profile_id)`), ~40 missing indexes (soft-delete partial indexes, FK lookups, composites, audit_log, server_error_report), 3 data migrations (populate `auth_backend`, backfill `file_library_sync`, generate `team_invitation.id` UUIDs)
- **0011**: Critical schema fixes — restructure `team_invitation` to UUID PK with nullable `team_id` + `CHECK(team_id IS NOT NULL OR org_id IS NOT NULL)` for org-level invitations; change `team_profile_rel.profile_id` FK from `ON DELETE CASCADE` to `ON DELETE RESTRICT`; recreate `sso_provider` with `domain NOT NULL` and `CHECK` constraints on `type` and `user_info_source`; update 8 application files to include `id: uuidv4()` in inserts
- **0013**: Page→file→project `modified_at` cascade trigger matching PG's `handle_page_update()`
- **0014**: Deletion protection triggers on 6 tables (`profile`, `team`, `file_thumbnail`, `file_tagged_object_thumbnail`, `file_media_object`, `team_font_variant`) using `server_prop` flag; storage object cascade triggers on `profile.photo_id`, `team.photo_id`, `file_media_object.media_id`/`thumbnail_id` hard-delete
- **0015**: JSONB expression indexes for storage dedup (`json_extract(metadata, '$.hash')`/`'$.bucket'`) and newsletter queries (`json_extract(props, '$.newsletter-news'/'newsletter-updates')`)

- Add Mattermost audit webhook reporter: forward audit events (logins, file operations, etc.) to a configurable Mattermost webhook via `PENPOT_AUDIT_WEBHOOK` env var
- Fix binfile media/asset embedding: `absorb-assets` now copies media entries from library files into the local file's `:media` map, ensuring images and fonts referenced by absorbed components are included in embedded exports
- Begin Phase 1 of Clojure/ClojureScript → ES JavaScript migration: port 7 foundational modules from `common/` to `shared/` — `i18n`, `version`, `path_names`, `buffer`, `perf`, `pprint`, `schema` (hand-built validators replacing Malli dependency)
- Continue Phase 1 migration: port 6 more modules from `common/` to `shared/` — `uri`, `spec` (spec/assertions replacing clojure.spec), `thumbnails`, `record` (defrecord → factory pattern), `data/macros` (select-keys, get-in, fmt), `data/undo_stack`
- Continue Phase 1 migration: port `weak` (WeakMap/WeakValueMap wrappers), `svg` (SVG constants, parse/format utilities), `svg/path/arc_to_bezier` (arc-to-Bezier conversion), `text` (DraftJS text conversion)
- Continue Phase 1 migration: port `files/variant` (variant component lookup), `files/comp_processors` (component repair/sync), `files/validate` (file validation), `logic/tokens` (token set operations stub), `logic/variant_properties` (variant property management), `logic/variants` (variant operations stub), `logic/shapes` (shape update/delete stub), `logic/libraries` (library sync stub), and add `types/variant.validVariantComponentQ` and `types/file.updateAllShapes`
- Continue Phase 1 migration: port `files/changes_builder` (changes builder with undo/redo, shape tree changes, library changes, design token changes) — the key blocker module that enables full implementation of logic modules
- Continue Phase 1 migration: port `files/changes` (change processing engine with 30+ change types including add/mod/del for objects, pages, colors, media, components, typography, tokens, guides, flows, grids, and plugin data). Fix hyphenated key handling in change destructuring throughout the module.
- Continue Phase 1 migration: add stub modules for `files/migrations`, `files/repair`, `files/builder`, `files/shapes_helpers`, `files/shapes_builder` (deeply coupled to PCB and type system — to be fully implemented in later phases)
- Continue Phase 1 migration: port `geom/line` (line equation utilities), `geom/proportions` (shape proportion setup), `geom/align` (alignment and distribution), `geom/snap` (snap point calculation), `geom/grid` (grid area calculation), `types/shape/interactions` (interaction types, event/action sets, overlay positioning), `types/token` (token type system, references, font weight aliases, combobox token parsing), `types/fills/impl` (binary fills encoding/decoding), `geom/modif_tree` (modification tree operations)
- Continue Phase 1 migration: port `types/shape/layout` (layout type constants, flex/grid layout predicates, sizing helpers, margins, alignment, scale functions), `geom/bounds_map` (bounds map creation and transformation), `geom/shapes/rect` (shape rect defaults), `geom/shapes/flex_layout/` (5 stub files), `geom/shapes/grid_layout/` (4 stub files), `geom/shapes/min_size_layout` (stub)
- Continue Phase 1 migration: complete `logic/variant_properties.js` — implement `generate-make-shapes-variant` (converts shapes into variant components with property handling)
- Complete Phase 1 migration: port `logging` (lightweight console-based logging API for JS environment, replacing SLF4J/CLJS macro system), `geom/modifiers` (geometry modifier propagation engine — the core module that propagates layout constraints, flex/grid layout modifiers, and auto-sizing calculations across the shape tree), and wire up all remaining barrel exports (`geom/shapes/flex_layout/*`, `geom/shapes/grid_layout/*`, `geom/shapes/min_size_layout`) in the main index. All 1255 tests passing across 132 suites. Phase 1 of the common/ → ES JS migration is now feature-complete
- Fix token key sets in types/token.js — `new Set(...spread)` was incorrectly creating sets from individual Set arguments instead of arrays
...
- Refactor switch/case dispatch patterns to Map-based lookups across shared, server, frontend, common, backend, and exporter modules for improved maintainability and O(1) lookup performance. Affected modules: modifiers, features, media, constraints, shape_type (shared); dispatcher, media, transit, scheduler (server); viewport/utils, http, serializers, shape_icon, parser, format, text_editor, fonts (frontend); shape, interactions, media, logging, features, variants (common); gc_touched, rlimit (backend); mime (exporter).
...
- **Fix transit decoder `__type` key leak**: The client's transit decoder was treating cognitect map encoding `["^ ", ...]` as a tagged value, causing the default `decodeTaggedMap` handler to inject a spurious `__type: " "` key into decoded objects. This corrupted the `pagesIndex` map in `get-file` responses, adding a phantom `__type` key alongside the real page UUIDs. The workspace then created a page with `id: "__type"` instead of the actual page ID, causing all `update-file` changes to be sent with `pageId: "__type"` — which the server's `handleAddObj` silently dropped (it couldn't find a page with that ID in `pagesIndex`). Result: shapes never persisted after save/reload. Fixed by moving the cognitect map `["^ "]` check before the generic tagged-map handler in `client/public/lib/transit.js`.
- **Fix server multipart file upload "Missing content" error**: The RPC dispatcher's multipart handler used `request.file()` which only processes the first part and doesn't expose all fields/part via the `data` object. This caused `params.content` to be undefined when the FormData had the file followed by text fields (fileId, name, isLocal). The `cmdUpload` function on the client sent the file as the first field, but the server's first-pass handler missed the text fields and the second-pass `request.parts()` iteration started from the second part, missing the file. Fixed by replacing the two-pass `request.file()` + `request.parts()` approach with a single `request.parts()` iteration that handles all parts uniformly in `server/src/rpc/dispatcher.js`.
- **Fix `penpot-notification` events never displayed as toasts**: The workspace emitted `penpot-notification` custom events for many actions (component created, plugin installed, import status, etc.) but nothing listened to them. The `penpot-notification.js` module's `info`/`success`/`warning`/`danger` functions were only called directly (e.g., `notifySuccess('File saved')`), missing all the `emit('penpot-notification', ...)` calls. Fixed by adding a document-level event listener in `penpot-notification.js` that catches `penpot-notification` events and dispatches to the appropriate toast function.
- **Fix layer panel never updated with shapes**: The workspace's `renderCurrentPage()` rendered shapes on the canvas but never updated the layer panel's `shapes` and `page` setters, so the layer panel always showed "No layers on this page." even after drawing shapes. Fixed by adding `layerPanel.page = page` and `layerPanel.shapes = Object.values(page.objects)` calls in `renderCurrentPage()` in `client/public/components/penpot-workspace.js`.
- **Fix share dialog calls non-existent RPC**: `penpot-share-dialog.js` called `update-file-share` which doesn't exist on the server. Replaced with `create-share-link` using a permissions array.
- **Fix version panel `this.escAttr is not a function`**: `penpot-version-panel.js` used `this.escAttr()` for attribute escaping but `PenpotElement` only had `escHtml()`. Added `escAttr()` method to `base.js` that escapes `&`, `"`, `'`, `<`, `>` for safe attribute interpolation.
- **Fix comments panel calls non-existent RPC**: `penpot-comment-panel.js` called `get-file-comments` which doesn't exist. Replaced with `get-comment-threads` using `file-id` parameter.
- **Fix deleted files calls non-existent RPC**: `penpot-dashboard.js` called `get-deleted-files`. Replaced with `get-team-deleted-files` using `teamId` parameter.
- **Fix `makeModifyChange` called with wrong arguments (5 locations)**: `penpot-workspace.js` called `makeModifyChange(shapeId, kvs)` passing a shape ID as the first argument (which should be `pageId`), leaving `shapeId` as `undefined`. This caused `Object.entries(undefined)` → `TypeError: Cannot convert undefined or null to object` in `createComponentFromSelection`, `detachSelectedInstance`, `syncSelectedInstance`, `deleteComponent`, and `startRichTextEdit`. Fixed all 5 locations to pass the correct `(pageId, shapeId, kvs)` arguments.
- **Fix `fs.createReadStream` not a function crash on thumbnail asset requests**: `server/src/http/assets.js` imported `node:fs/promises` which has no `createReadStream()` method. The asset route crashed with `fs.createReadStream is not a function` when serving any file from disk. Fixed by importing both `node:fs/promises` (for `stat`) and `node:fs` (for `createReadStream`).
