 ...
 */
/**
 * Represents a message sent through the plugin API bridge.
 * The type field identifies the kind of message, and the payload
 * carries the request or response data.
 */
export interface BridgeMessage {
  /**
   * The type of the bridge message.
   */
  readonly type: string;
  /**
   * The payload of the bridge message.
   */
...
/**
 * The plugin API bridge provides a structured communication channel
 * between the plugin runtime and the canvas host application.
 * It enables request/response patterns and event-based messaging.
 *
 * Requires `allow:api-bridge` permission.
 *
 * @example
 * ```js
 * // Send a command to the host and receive a response
 * const response = await penpot.bridge.send('get-custom-data', { key: 'myKey' });
 *
 * // Listen for commands from the host
 * penpot.on('bridgecommand', (message) => {
 *   if (message.type === 'update-widget') {
 *     // handle the command
 *   }
 * });
 * ```
...
export interface BridgeContext {
  /**
   * Sends a message to the host application through the bridge and
   * returns a promise that resolves with the host's response.
   *
   * @param type The type identifying the kind of bridge message.
   * @param payload The payload data to send with the message.
   * @returns A promise that resolves with the response from the host.
   *
   * @example
   * ```js
   * const result = await penpot.bridge.send('fetch-external-data', { url: 'https://example.com/data' });
   * console.log(result);
   * ```
   */
  ...
  /**
   * Sends a one-way notification to the host application.
   * Unlike `send`, this does not wait for a response.
   *
   * @param type The type identifying the kind of bridge message.
   ...
   * @example
   * ```js
   * penpot.bridge.notify('plugin-ready', { version: '1.0' });
   * ```
   */
...
> {
  ...
  utils: ContextUtils;
  /**
   * The plugin API bridge for structured communication between
   * the plugin runtime and the host application.
   * Requires `allow:api-bridge` permission.
   *
   * @example
   * ```js
   * const response = await penpot.bridge.send('get-data', { key: 'value' });
   * ```
   */
...
export interface EventsMap {
  ...
  /**
   * The `bridgecommand` event is triggered when a bridge command is received
   * from the host application. Requires `allow:api-bridge` permission.
   */
  bridgecommand: BridgeMessage;
}
